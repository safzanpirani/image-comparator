document.addEventListener("DOMContentLoaded", () => {
  ...
  function initializeApiKey() {
    const savedKey = storageType.getItem("groq_api_key");
    if (savedKey) {
  ...
  // Initialize the API key on page load
  initializeApiKey();
  ...
  function loadConversationHistory() {
    try {
      const savedHistory = storageType.getItem("chat_history");
      if (savedHistory) {
  ...
  function showApiKeyError(message) {
    ...
    document
      .getElementById("image-to-resize")
    ...
    const colorAdjustmentForm = document.getElementById(
    ...
    contrastSlider.addEventListener("input", () => {
      document.getElementById("contrast-value").innerText =
            ...
            for (let i = 0; i < pixels.data.length; i += 4) {
              ...
              if (delta !== 0) {
                ...
                r = Math.max(
                  ...
                  Math.min(
                    255,
                    ((r - min) / delta) * (newMax - newMin) + newMin,
                  ),
                );
                g = Math.max(
                  0,
                  Math.min(
                    255,
                    ((g - min) / delta) * (newMax - newMin) + newMin,
                  ),
                );
                b = Math.max(
                  0,
                  Math.min(
                    255,
                    ((b - min) / delta) * (newMax - newMin) + newMin,
                  ),
                );
              }
    ...
    document
      .getElementById("image-for-curves")
